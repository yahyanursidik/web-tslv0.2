...
const registerUrl = `${whatsapp}?text=${encodeURIComponent('Assalamu’alaikum, saya ingin mendaftar program Ma’had Pendidikan Bahasa Arab dan Diniyah TSL Daring. Mohon informasi pendaftarannya.')}`;
const consultUrl = `${whatsapp}?text=${encodeURIComponent('Assalamu’alaikum, saya ingin berkonsultasi mengenai program Ma’had TSL Daring.')}`;
...
const faqs = [
  ...
  ['Apakah pemula dapat mengikuti program ini?', 'Dapat. Tes penempatan membantu menentukan marhalah yang sesuai, sehingga peserta memulai dari tingkat kemampuan yang tepat.'],
  ['Berapa lama satu periode pembelajaran?', 'Program berjalan dengan sistem semester dan marhalah. Jadwal serta durasi terbaru diinformasikan pada saat pendaftaran.'],
...
export default function MahadTSL() {
  ...
  return (
    <main className="bg-[#fbfaf4] pb-20 text-[#1d2a20] sm:pb-0">
        ...
        <div className="relative isolate overflow-hidden rounded-[30px] bg-[#183d25] text-white shadow-[0_24px_70px_rgba(24,61,37,.17)]">
          <div className="grid lg:min-h-[620px] lg:grid-cols-[1.05fr_.95fr]">
            <div className="flex flex-col justify-center px-6 py-12 sm:px-10 lg:px-14 lg:py-16">
              ...
              <p className="mt-6 max-w-xl text-base leading-8 text-white/70 sm:text-lg">Program pembelajaran Bahasa Arab dan ilmu diniyah yang sistematis, bertahap, dan berbasis marhalah untuk membangun fondasi keilmuan Islam berdasarkan Al-Qur’an dan As-Sunnah.</p>
      ...
      <section id="keunggulan" className="site-container scroll-mt-36 py-20 sm:py-24"><div className="text-center"><span className="eyebrow">Mengapa Ma’had TSL?</span><h2 className="mx-auto mt-5 section-title">Bukan sekadar selesai materi.</h2><p className="mx-auto mt-4 section-copy">Program dirancang untuk menumbuhkan pemahaman yang runtut, terukur, dan dapat dipertanggungjawabkan.</p></div><div className="mt-12 grid gap-5 md:grid-cols-3">{[
        ['account_tree','Terstruktur','Kurikulum disusun sistematis dari tingkat dasar menuju tingkat lanjutan melalui marhalah.'],
        ['trending_up','Bertahap','Setiap materi dikuasai sebelum peserta berpindah ke tingkat pembelajaran berikutnya.'],
        ['verified','Mendalam','Pembelajaran menekankan konteks, kaidah, dan pemahaman—bukan hafalan semata.'],
      ].map(([icon,title,text],index) => <article key={title} className="group rounded-2xl border border-[#d8ddd1] bg-white p-7 transition-all hover:-translate-y-1 hover:border-[#9caf94] hover:shadow-[0_16px_36px_rgba(24,61,37,.09)]"><div className="flex items-center justify-between"><span className="grid h-12 w-12 place-items-center rounded-xl bg-[#e3efd8] text-[#285c32] transition-colors group-hover:bg-[#285c32] group-hover:text-white material-symbols-outlined">{icon}</span><span className="font-headline text-4xl font-black text-[#e5e9e1]">0{index+1}</span></div><h3 className="mt-7 font-headline text-2xl font-bold text-[#183d25]">{title}</h3><p className="mt-3 text-sm leading-7 text-[#697264]">{text}</p></article>)}</div></section>

      <section id="kurikulum-mahad" className="scroll-mt-36 bg-[#eef0e7] py-20 sm:py-24"><div className="site-container grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-14"><div><span className="eyebrow bg-white">Kurikulum Ma’had</span><h2 className="mt-5 section-title">Enam rumpun ilmu dalam satu jalur.</h2><p className="mt-4 section-copy">Bahasa menjadi alat untuk memahami sumber ilmu, sementara materi diniyah menata pemahaman dan amal secara seimbang.</p><div className="mt-8 rounded-2xl bg-[#285c32] p-6 text-white"><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#f0c84b]">Sistem marhalah</p><p className="mt-3 text-sm leading-7 text-white/70">Tes penempatan membantu peserta memulai pada level yang sesuai—tidak terlalu mudah, tidak pula melampaui fondasi yang dibutuhkan.</p></div></div><div className="grid gap-4 sm:grid-cols-2">{subjects.map(([icon,title,text]) => <article key={title} className="flex gap-4 rounded-2xl border border-[#d8ddd1] bg-white p-5"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#e3efd8] text-[#285c32] material-symbols-outlined">{icon}</span><div><h3 className="font-headline text-lg font-bold text-[#183d25]">{title}</h3><p className="mt-1 text-xs leading-6 text-[#697264]">{text}</p></div></article>)}</div></div>
      ...
      <section id="pendaftaran-mahad" className="site-container scroll-mt-36 py-20 sm:py-24"><div className="text-center"><span className="eyebrow">Alur pendaftaran</span><h2 className="mx-auto mt-5 section-title">Empat langkah menuju kelas pertama.</h2><p className="mx-auto mt-4 section-copy">Setiap tahap membantu memastikan peserta berada pada kelas dan jalur belajar yang tepat.</p></div><div className="relative mx-auto mt-12 max-w-5xl"><div className="absolute left-[10%] right-[10%] top-8 hidden h-px bg-[#bec7ba] md:block"/><div className="relative grid gap-5 md:grid-cols-4">{[
        ...
        ['02','Tes penempatan','Ikuti ujian daring untuk menentukan marhalah.','assignment'],
      ...
      <section id="biaya-mahad" className="scroll-mt-36 bg-[#183d25] py-20 text-white sm:py-24"><div className="site-container"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><span className="inline-flex rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-extrabold uppercase tracking-[.13em] text-[#f0c84b]">Investasi ilmu</span><h2 className="mt-5 font-headline text-3xl font-black sm:text-4xl">Transparan sejak awal.</h2><p className="mt-4 max-w-md text-sm leading-7 text-white/65">Biaya mendukung layanan pembelajaran digital dan pengelolaan kelas yang terarah. Konfirmasikan nominal periode berjalan kepada admin.</p></div><div className="overflow-hidden rounded-[26px] bg-white text-[#1d2a20]"><div className="grid sm:grid-cols-2"><div className="p-7 sm:p-8"><p className="text-[10px] font-extrabold uppercase tracking-[.13em] text-[#7a8175]">Pendaftaran & tes</p><strong className="mt-3 block font-headline text-3xl text-[#183d25]">Rp150.000</strong><p className="mt-2 text-xs text-[#697264]">Dibayarkan pada proses seleksi awal.</p></div><div className="border-t border-[#d9ddcf] p-7 sm:border-l sm:border-t-0 sm:p-8"><p className="text-[10px] font-extrabold uppercase tracking-[.13em] text-[#7a8175]">SPP per semester</p><strong className="mt-3 block font-headline text-3xl text-[#183d25]">Mulai Rp1.200.000</strong><p className="mt-2 text-xs text-[#697264]">Menyesuaikan marhalah dan periode program.</p></div></div><div className="flex gap-3 border-t border-[#d9ddcf] bg-[#eff8e9] p-5 sm:px-8"><span className="material-symbols-outlined text-xl text-[#285c32]">volunteer_activism</span><p className="text-xs leading-6 text-[#596358]"><strong className="text-[#183d25]">Beasiswa tersedia</strong> bagi santri berprestasi dan kurang mampu, sesuai syarat, kuota, dan ketentuan.</p></div></div></div></div></section>
